...
export default defineContentScript({
    ...
    async main(ctx) {
        ...
        const mainTimer = setInterval(async () => {
            ...
            const isInPinned = await isPageInPinnedHistory(location.href);
            const isAuto = await browser.storage.local.get({
                isAutoHandle: true,
            });

            let runControl =
                (isCol && isAuto.isAutoHandle) ||
                (isCol && isInPinned && isAuto.isAutoHandle == false);

            if (runControl !== isPageReady()) {
                setIsPageReady(!!runControl);
                setIsAutoHandle(!!isAuto.isAutoHandle);
                if (runControl) {
                    browser.runtime
                        .sendMessage({
